...
sub waitForFile {
    ...
    $self->nest("waiting for file ‘$fileName’", sub {
        retry sub {
          #my ($status, $out) = $self->execute("tree `pwd` >&2; rpcinfo -p >&2; systemctl status >&2; lsof >&2; lslocks >&2; lsof /data/lock >&2; journalctl -xe >&2; ls -la /data >&2; test -e $fileName >&2");
            my ($status, $out) = $self->execute("test -e $fileName >&2");
            return 1 if $status == 0;
        }
    });
}

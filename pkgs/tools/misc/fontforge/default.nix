...
, libungif, libpng, libjpeg, libtiff, libxml2, patchelf
, withX11 ? false
...
stdenv.mkDerivation {
  ...
  buildInputs =
    [ gettext freetype zlib libungif libpng libjpeg libtiff libxml2 patchelf ]
    ++ lib.optionals withX11 [ libX11 xproto libXt ];

  # libpng is loaded on-demand
  postFixup = ''
    patchelf --set-rpath `patchelf --print-rpath "$out/bin/fontforge"`:"${libpng}/lib" "$out/bin/fontforge"
  '';
}

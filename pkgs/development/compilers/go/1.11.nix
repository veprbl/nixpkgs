...
stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "go";
    rev = "go${version}";
    sha256 = "1adxjsrb7j120x9sbal03zj9wxfckiqlbzvdn0wpj4w7l1l1cfas";
  };

  # perl is used for testing go vet
  nativeBuildInputs = [ perl which pkgconfig patch makeWrapper procps git ];
  ...
  prePatch = ''
    ...
    sed -i 's,/usr/bin,'"`pwd`", src/cmd/vendor/golang.org/x/sys/unix/syscall_unix_test.go
    ...
    # Disable cgo user tests not working
    rm src/os/user/user_test.go
  ...
  configurePhase = ''
    ...
    echo "$version" > $out/share/go/VERSION
  '';
